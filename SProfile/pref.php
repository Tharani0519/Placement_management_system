...
if($USN !=''||$email !='')
{
	if($USN == $sun)
    {
    if($query = $connect->query("INSERT INTO `revised`.`basicdetails` ( `FirstName`, `LastName`, `USN`, `Mobile`, `Email`, `DOB`, `Sem`, `Branch`, `SSLC`, `PU/Dip`, `BE`, `Backlogs`, `HofBacklogs`, `DetainYears`, `Approve`)
...
if($USN !=''||$email !='')
{
	...
	$sql = $connect->query("SELECT * FROM `revised`.`basicdetails` WHERE `USN`='$USN'");
	if($sql->num_rows == 1)
	{

		if($query = $connect->query("UPDATE `revised`.`basicdetails` SET `FirstName`='$fname', `LastName`='$lname', `Mobile`='$phno', `Email`='$email', `DOB`='$date', `Sem`='$cursem', `Branch`= '$branch', `SSLC`='$per', `PU/Dip`='$puagg', `BE`='$beaggregate', `Backlogs`='$back', `HofBacklogs`='$hisofbk', `DetainYears`='$detyear' ,`Approve`='0'
